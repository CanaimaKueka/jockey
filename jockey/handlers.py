...
class Handler:
    ...
    def hw_description(self):
        if self._modinfo:
            device_info = ''
            if len(self._hwids) > 0:
                device_info = self._hwids[0].text_info()
            driver_info = {
                'license': ','.join(self._modinfo.get('license', [])),
                'author': ','.join([s.decode('utf-8') for s in self._modinfo.get('author', [])]),
                'version': ','.join(self._modinfo.get('version', [])),
                'depends': ','.join(self._modinfo.get('depends', [])),
                'firmware': ','.join(self._modinfo.get('firmware', [])),
            }
            return json.dumps({
...
class DummyHandler(Handler):
    def __init__(self, *args, **kwargs):
        Handler.__init__(
            ...
            rationale=kwargs.get('rationale', None)
        )

    def id(self):
        if self._modinfo:
            return 'dummy:%s' % (
                '_'.join(self._modinfo.get('description', [])).replace(' ', '_')
            ) 

    def name(self):
        if self._modinfo:
            return ','.join(self._modinfo.get('description', []))
...
class ModaliasOverrideHandler(FirmwareHandler):
    '''Class for Modaliases Overrides, installs a package and activate the
    correct module'''

    def __init__(self, backend, package, kernel_module, testfile, name=None, 
            description=None, rationale=None, url=None, sha1sum=None, free=False):
    ...
    def enable(self):
        return KernelModuleHandler.enable(self)
