...
class UnknownHandlerException(dbus.DBusException):
    _dbus_error_name = '%s.UnknownHandlerException' % (DBUS_BUS_NAME, )


class InvalidModeException(dbus.DBusException):
    _dbus_error_name = '%s.InvalidModeException' % (DBUS_BUS_NAME, )


class InvalidDriverDBException(dbus.DBusException):
    _dbus_error_name = '%s.InvalidDriverDBException' % (DBUS_BUS_NAME, )


class PermissionDeniedByPolicy(dbus.DBusException):
    _dbus_error_name = '%s.PermissionDeniedByPolicy' % (DBUS_BUS_NAME, )
...
class Backend(dbus.service.Object):
    ...
    def handler_info(self, handler_id, sender=None, conn=None):
        ...
        info = {
            ...
            'ignored': str(self.handler_is_ignored(h.id())),
            'dummy': str(isinstance(h, detection.handlers.DummyHandler)),
        }
        ...
        for f in ['package', 'repository', 'repository_sign_fp',
                'driver_vendor', 'version', 'license', 'hw_description']:
            v = getattr(h, f)
            if v:
                info[f] = v
    ...
    def set_enabled(self, handler_id, enable, sender=None, conn=None):
        ...
        # If a driver is disabled, add it to the check cache as ignored
        if not enable:
            logging.debug('Updating check cache')
            used, seen, ignored, session_id = self.check_cache()
            ignored.update([handler_id])
            logging.debug('Check cache: used -> %s, seen -> %s, ignored -> %s' % (used, seen, ignored))
            self.write_cache(used, seen, ignored, session_id)
    ...
    @dbus.service.method(DBUS_INTERFACE_NAME,
        in_signature='sb', out_signature='b', sender_keyword='sender',
        connection_keyword='conn')
    def handler_set_ignored(self, h_id, ignore, sender=None, conn=None):
        used, seen, ignored, session_id = self.check_cache()
        if ignore:
            logging.warning('Handler %s will be ignored in notifications' % (h_id, ))
            ignored.add(h_id.strip())
        else:
            logging.warning('Handler %s will be included in notifications' % (h_id, ))
            ignored.remove(h_id.strip())
        self.write_cache(used, seen, ignored, session_id)
        return True

    def handler_is_ignored(self, h_id):
        '''
        Check if a handler is ignored in the check cache
        '''
        used, seen, ignored, session_id = self.check_cache()
        return h_id in ignored

    def check_cache(self):
        '''
        Reads the check cache and returns a tuple with (used, ignored) handlers
        '''
        # read previously seen/ignored/used handlers
        seen = set()
        ignored = set()
        used = set()
        session_id = None
        ...
        if os.path.exists(OSLib.inst.check_cache):
            with open(OSLib.inst.check_cache) as f:
                for line in f:
                    try:
                    ...
                    if flag == 'ignored':
                        ...
                        logging.debug('check_cache: %s, %s' % (flag, h))
                        try:
                            h, session_id = h.split()
                        except ValueError:
                            session_id = ''
                        seen.add(h.strip())
                    else:
        ...
        return (used, seen, ignored, session_id)

    def write_cache(self, used, seen, ignored, session_id):
        '''
        Write handlers to the check cache
        '''
        logging.debug('Writing cache with session id %s' % session_id)
        used = used.difference(ignored)
        logging.debug(
            'Writing check cache %s: (used: %s, seen: %s, ignored: %s)',
        ...
        if not os.path.exists(os.path.dirname(OSLib.inst.check_cache)):
            os.makedirs(os.path.dirname(OSLib.inst.check_cache))
        with open(OSLib.inst.check_cache, 'w') as f:
            for i in ignored:
                print >> f, 'ignored', i
            for u in used:
                print >> f, 'used', u
            for s in seen:
                print >> f, 'seen', s, session_id

    @dbus.service.method(DBUS_INTERFACE_NAME,
        in_signature='ss', out_signature='(asas)', sender_keyword='sender',
        connection_keyword='conn')
    def new_used_available(self, mode='any', session_id='', sender=None, conn=None):
        '''
        ...
        used, seen, ignored, old_session_id = self.check_cache()
        ...
        for h_id, h in self.handlers.iteritems():
            ...
            if h_id not in seen:
                new_avail.add(h_id)
                logging.debug('Handler %s was prevously unseen, now seen', h_id)
            if h_id not in ignored:
                avail.add(h_id)
                logging.debug('Handler %s is available', h_id)
            if h_id not in used and h.used():
                new_used.add(h_id)
                logging.debug('Handler %s previously unused marked as used', h_id)

        # Do not show seen handlers if session ids match
        if old_session_id == session_id:
            new_used = new_used.difference(seen)

        # write back cache
        used.update(new_used)
        seen.update(new_avail)
        logging.debug('write_cache: %s', ((used, seen, ignored, session_id), ))
        self.write_cache(used, seen, ignored, session_id)

        # throw out newly available handlers which are already enabled, no need
        # to bother people about them
        for h_id in list(avail): # create a copy for iteration
            try:
        ...
        logging.debug('new_used_available: %s' % ((new_used, new_avail), ))
    ...
    def create_dbus_client(klass, session_bus=False):
        ...
        return dbus.Interface(obj, DBUS_INTERFACE_NAME)
