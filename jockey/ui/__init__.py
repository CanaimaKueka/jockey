...
from jockey.backend import UnknownHandlerException, PermissionDeniedByPolicy, \
...
class AbstractUI(dbus.service.Object):
    ...
    def init_strings(self):
        ...
        self.string_label_ignore = self._('Ignore', True)
        ...
        self.string_unsupported_driver = self._('Driver Unsupported')
        self.string_show = self._('Show')
    ...
    def main_window_text(self):
        ...
        for h_id in self.backend().available(self.argv_options.mode):
            info = self._handler_info_cache.get(h_id, None) or self.backend().handler_info(h_id)
            # print 'main_window_text: info for', h_id, info
            if not bool(info['dummy']):
    ...
    def get_ui_driver_name(self, handler_info):
        ...
        if bool(handler_info['dummy']):
            result += ' (%s)' % self.string_unsupported_driver
    ...
    def get_ui_driver_info(self, handler_id):
        ...
        if not handler_id:
            return {
                'name': '', 'description': '', 'free': None, 
                'enabled': None, 'used': None, 'license_text': '',
                'status_label': '', 'license_label': '', 'certified': None,
                'certification_label': '', 'button_toggle_label': None,
                'ignore_label': self.string_label_ignore,
                'ignored': False
            }

        info = self._handler_info_cache.get(handler_id, None) or self.backend().handler_info(handler_id)
        ...
        result = {
            ...
            'ignored': bool(info['ignored']),
            'used': bool(info['used']),
            'needs_reboot': False,
            'license_text': info.get('license', ''),
            'dummy': bool(info['dummy'])
        }
        ...
        if result['enabled']:
            ...
            if result['used']:
                ...
                if result['ignored']:
                    result['status_label'] = self._('This driver is not activated and is ignored in notifications.')
                else:
                    result['status_label'] = self._('This driver is not activated.')

                if result['dummy']:
                    result['status_label'] = self._('This driver is not supported.')
    ...
    def parse_argv(self):
        ...
        parser.add_option ('-s', '--session-id',
                dest='session_id',
                help=self._('Set a session id for notifications.'))
    ...
    def run(self):
        ...
        if self.argv_options.enable:
            ...
            ret = 0
            for h_id in self.backend().available(self.argv_options.mode):
                i = self._handler_info_cache.get(h_id, None) or self.backend().handler_info(h_id)
    ...
    def check(self):
        ...
        session_id = ''
        if self.argv_options.session_id:
            session_id = self.argv_options.session_id

        try:
            print(self.argv_options.mode, session_id)
            (new_used, avail) = convert_dbus_exceptions(
                self.backend().new_used_available,
                self.argv_options.mode,
                session_id
            )
        ...
        for h_id in set(avail): # create copy
            info = self._handler_info_cache.get(h_id, None) or self.backend().handler_info(h_id)
    ...
    def set_handler_enable(self, handler_id, action, confirm, gui=True):
        ...
        '''
        try:
            i = self._handler_info_cache.get(handler_id, None) or convert_dbus_exceptions(self.backend().handler_info, handler_id)
        ...
        try:
            ...
            self.error_message('Error during installation', str(e))
            return False
        finally:
            if self._install_progress_shown and gui:
                self.ui_progress_finish()

        # Do not use _handler_info_cache here. Refresh needed
        info = self.backend().handler_info(handler_id)
        newstate = bool(info['enabled'])
        ...
        if newstate:
            self.ui_notification(
                self._('Installation Successful'),
                text=self._(
                    'Driver for %s was successfully installed.'
                ) % info['name'],
                show_button=None
            )
    ...
    def toggle_handler_ignore(self, h_id):
        try:
            info = self._handler_info_cache.get(h_id, None) or convert_dbus_exceptions(
                self.backend().handler_info,
                h_id
            )
            ignored = bool(info['ignored'])
            self.backend().handler_set_ignored(h_id, not ignored)
        except Exception as e:
            self.error_message(
                '', 'Error while ignoring %s\n%s' % (
                    h_id, str(e)
                )
            )
            raise
    ...
    def _get_description_rationale_text(self, h_info):
        ...
        if d and r:
            ...
            # If no description is found in the handler, build one
            # out of the available information
            desc = ''

            if 'hw_description' in h_info:
                hwinfo = json.loads(h_info['hw_description'])

                driver_info = _(u'''Driver Information:
License: %(license)s
Author: %(author)s
Version: %(version)s
...
Firmware: %(firmware)s
''')
                driver_info = driver_info % hwinfo['driver_info']
                ...
                if h_info.get('dummy', False):
                    dummy_description = _('''This device is not supported for
automated installation and we have found
that this is probably not active in the system.\n\n
The following information was automatically
collected so you can use it to research
about the device.
''')
                    desc = dummy_description + desc
    ...
    def search_driver(self, hwid, sender=None, conn=None):
        ...
        if self.current_search[1]:
            ...
            for d in self.current_search[1]:
                info = self._handler_info_cache.get(d, None) or self.backend().handler_info(d)
    ...
    def ui_notification(self, title, text):
        ...
        raise NotImplementedError('notification support not available: %s' \
