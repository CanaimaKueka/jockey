...
class OSLib:
        ...
        if out:
            for l in out.splitlines():
                if l.startswith('Section:'):
                    s = l.split()[-1]
                    return not s.startswith('non-free')
    ...
    def install_package(self, package, progress_cb, repository=None,
        ...
        '''
        class MyFetchProgress(apt.progress.base.AcquireProgress):
    ...
    def ignored_modules(self):
        ...
        # Do not ignore modules in a modalias override file provided by jockey
        for alias_location in self.modaliases_overrides:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                continue
                    logging.debug('OSLib.ignored_modules: found %s in modalias override, removing from ignored_modules' % m)
                    if m in result:
                        result.remove(m)
    ...
    def package_header_modaliases(self, cache=None):
        ...
        for package in cache:
            # if package.candidate.architecture != system_architecture:
            #     continue
            try:
