...
'''A list of device subsystems that will be scanned to detect devices'''
SUBSYSTEMS = ['pci', 'pcmcia', 'usb', 'scsi']
...
class HardwareID:
    ...
    def __init__(self, type, id):
        self.type = type
        self.id = id
        self._pci_modalias_re = re.compile(self.pci_modalias_re_str)
        self._usb_modalias_re = re.compile(self.usb_modalias_re_str)
        self._parsed_modalias = None
    ...
    def parse_modalias(self):
        if self.type == 'modalias':
            if not self._parsed_modalias:
                if self.device_type() == 'pci':
                    self._parsed_modalias = [
                        i.groupdict()
                        for i in
                        self._pci_modalias_re.finditer(
                            self.modalias()
                        )
                    ][0]
                    ...
                    self._parsed_modalias = [
                        i.groupdict()
                        for i in
                        self._usb_modalias_re.finditer(
                            self.modalias()
                        )
                    ][0]
            return self._parsed_modalias

    def modalias_part(self, part):
        ret = ''
        if self.type == 'modalias':
            ret = self.parse_modalias()[part]
        return ret
    ...
    def text_info(self):
        if self.device_type() == 'pci':
            return (
                ...
                '\n\tSubvendor ID: %s\n') % (
                self.device_type(),
                self.modalias_part('device'),
                self.modalias_part('subdevice'),
                self.modalias_part('vendor'),
                self.modalias_part('subvendor'),
            )
        elif self.device_type() == 'usb':
            return (
                '\tType: %s'
...
class LocalKernelModulesDriverDB(DriverDB):
    ...
    def _do_query(self, hwid):
        ...
        logging.debug('LocalKernelModulesDriverDB._do_query(%s): result -> %s'% (hwid, [res.properties for res in result], ))
        return result
...
def _driverid_to_handler(did, backend, mode):
    ...
    _modinfo = None
    if 'kernel_module' in did:
        _modinfo = get_modinfo(did['kernel_module'])
    ...
    if did['jockey_handler'] in ('KernelModuleHandler', 'FirmwareHandler') and \
        ...
        logging.debug('Looking for a handler for %s' % (did, ))
        ...
        if not hclass:
            logging.debug('No custom handler, fall back to creating a standard one (%s)' % (did, ))
            ...
            if did['kernel_module'] not in _driverid_to_handler.ignored:
                ...
                logging.debug('Not in ignored_modules: %s\tModinfo: %s' % (did, _modinfo))
                if not _modinfo:
    ...
    using_dummy_handler = False
    if not hclass:
        firmware_check = _modinfo \
            and 'firmware' in _modinfo \
            and False in [
                os.path.exists(os.path.join('/lib/firmware', fw))
                for fw in _modinfo['firmware']
            ]
        if firmware_check:
            logging.debug('Support not found for %s' % (did.properties, ))
            logging.warning('Using DummyHandler for %s %s' % (args, kwargs))
            hclass = handlers.DummyHandler
            using_dummy_handler = True
        else:
...
def get_modinfo(module):
    ...
    modaliases).
    '''
    # Use a cache to reduce system calls
    try:
...
def get_db_handlers(backend, db, hardware, mode=MODE_ANY):
    ...
    for hwid in hardware:
        # SSB bus devices are already detected as PCI devices,
        # so it's safe to skip them
        if hwid.device_type() == 'ssb':
            continue
